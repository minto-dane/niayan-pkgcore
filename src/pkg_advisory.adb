...
package body Pkg_Advisory with SPARK_Mode is
   function Valid (A : Advisory) return Boolean is
     (A.ID /= Zero_Digest and then A.Package_ID /= Zero_Digest and then A.Fixed_Build /= Zero_Digest
      ...
      and then (not A.Authority_Reports_Wild_Exploitation or else
        (A.Exploitation_Report_Authenticated and then A.Reporting_Authority /= Zero_Digest
         and then A.Exploitation_Report_Record /= Zero_Digest and then A.Exploitation_Report_Published_At > 0))
   ...
   begin
      ...
      if A.Authority_Reports_Wild_Exploitation and then A.Level >= Important then return Emergency_Change;
