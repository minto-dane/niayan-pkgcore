...
package body Pkg_Versions with SPARK_Mode is
   ...
   begin
      ...
      loop
         while L <= Left'Last and then not Special (Left (L)) loop L := L + 1; end loop;
         while R <= Right'Last and then not Special (Right (R)) loop R := R + 1; end loop;
         ...
         then
            ...
            LE := L; RE := R;
            if Numeric then
               while LE <= Left'Last and then Digit (Left (LE)) loop LE := LE + 1; end loop;
               while RE <= Right'Last and then Digit (Right (RE)) loop RE := RE + 1; end loop;
            else
               while LE <= Left'Last and then Alpha (Left (LE)) loop LE := LE + 1; end loop;
               while RE <= Right'Last and then Alpha (Right (RE)) loop RE := RE + 1; end loop;
            end if;
            if RE = R then
            ...
            LS := L; RS := R;
            if Numeric then
               while LS < LE and then Left (LS) = '0' loop LS := LS + 1; end loop;
               while RS < RE and then Right (RS) = '0' loop RS := RS + 1; end loop;
