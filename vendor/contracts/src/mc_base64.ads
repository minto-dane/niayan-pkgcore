...
package MC_Base64 with SPARK_Mode, Pure is
   function Encode(Data : Bytes) return String with Global=>null,Pre=>Data'Length<=1_048_576,
     Post => Encode'Result'First=1 and then Encode'Result'Length=4*((Data'Length+2)/3);
   procedure Decode(Text : String; Data : out Bytes; Used : out Natural; Status : out Outcome) with Global=>null,Post=>Used<=Data'Length;
