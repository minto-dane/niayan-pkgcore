...
SOURCES
apt-get update
apt-get install -y --no-install-recommends ca-certificates curl dbus dpkg gcc git gnat gpg gpg-agent gpgconf gpgv gprbuild libarchive-dev libc6-dev libcurl4-openssl-dev libsodium-dev libsystemd-dev libxml2-dev make pkg-config python3 python3-cryptography python3-tuf ripgrep xz-utils zstd
